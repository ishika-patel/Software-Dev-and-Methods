...
function runPage() {
  ...
  // add in function here and you can call the helper
  getCoordinates();
  ...
  $.ajax({url:url, dataType:"jsonp"}).then(function(data) {
    ...
    */
    ...
    $('#local_time').html(data.location.localtime);
        ...
        $('#thermometer_inner').css('height',getTempBar(data.current.temperature));

        if(getFarenheitTemp(data.current.temperature) < 65) 
        {
          $('#thermometer_inner').css('background',"blue");
        }
        else if(getFarenheitTemp(data.current.temperature) > 85) 
        {
   ...
   for(var i = 1; i <= 5; i++) {
     dayKey = getKey(i);
     nextDayForecast = '<div style="width: 20%;"><div class="card"><div class="card-body"><h5 class="card-title">'+dayOfWeek+'</h5>';
     nextDayForecast += '<p class="card-text">High: '+getFarenheitTemp(dayKey.maxtemp)+'<br>Low: '+getFarenheitTemp(dayKey.mintemp)+'<br>';
     nextDayForecast += 'Sunrise: '+dayKey.astro.sunrise+'<br>Sunset: '+dayKey.astro.sunset+'</p></div></div></div>';
     FiveForecast += nextDayForecast;
   }

   $('#5_day_forecast').html(FiveForecast);
