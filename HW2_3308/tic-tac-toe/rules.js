...
function begin_play()
{
	...
	if(document.getElementById("player2_id").value.length == 0 || document.getElementById("player1_id").value.length == 0)
	{
		alert("Please fill in both names before starting game!");
		return;
	} 
	else 
	{
		...
		document.getElementById("turn_info").innerHTML = (this.turn) ? "Turn for: <b>X</b>" : "Turn for: <b>O</b>";
		this.started = true;
	}
}
...
EXTRA FUNCTIONS: function to check move is legal, checking for winner based on different winning patterns then add alert, updating board function
*/
...
function legalMove() 
{
	...
	{
		if (document.getElementById("move_text_id").value == this.table_ids[i] && this.board_state[i] == -1)
		 {
			return i;
		}
	}
...
function updateBoard(moveIndex)
 {
	...
	this.board_state[moveIndex] = (this.turn) ? 1 : 0;
	document.getElementById(table_ids[moveIndex]).innerHTML = (this.turn) ? "X" : "O";
	this.turn = !turn;
	document.getElementById("turn_info").innerHTML = (this.turn) ? "Turn for: <b>X</b>" : "Turn for: <b>O</b>";
}
...
function checkForWinner() 
{
	for(let i = 0; i < 3; i++) 
	{
		if(this.board_state[i] != -1 && this.board_state[i] == this.board_state[i+3] && this.board_state[i] == this.board_state[i+6]) 
		{
			var winner = (this.board_state[i]) ? "X" : "O";
	...
	for(let i = 0; i <= 6; i+=3) 
	{
		if(this.board_state[i] != -1 && this.board_state[i] == this.board_state[i+1] && this.board_state[i] == this.board_state[i+2]) 
		{
			var winner = (this.board_state[i]) ? "X" : "O";
			alert("Winner is " + winner);
			reset_play();
	...
	if(this.board_state[0] != -1 && this.board_state[0] == this.board_state[4] && this.board_state[0] == this.board_state[8]) 
	{
		var winner = (this.board_state[0]) ? "X" : "O";
		alert("Winner is " + winner);
		reset_play();
	}
	if(this.board_state[2] != -1 && this.board_state[2] == this.board_state[4] && this.board_state[2] == this.board_state[6]) 
	{
		var winner = (this.board_state[2]) ? "X" : "O";
		alert("Winner is " + winner);
		reset_play();
	}
}
